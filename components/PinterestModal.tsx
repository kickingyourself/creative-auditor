...
/**
 * components/PinterestModal.tsx
 * Tab order: Pinterest URL (default) → Upload File
 * Pinterest has a live API so URL comes first.
 */
...
function PinIcon({ size = 15, color = "#e60023" }: { size?: number; color?: string }) {
  return (
    ...
    startTransition(async () => {
      ...
      try {
        prepRes = await fetch("/api/upload/prepare", {
          ...
          body: JSON.stringify({
            brand_id: brandId, platform: "pinterest",
        ...
        const reg = await fetch("/api/upload/register", {
          ...
          body: JSON.stringify({
            ...
            items: successes.map((u: { storagePath: string; contentType: string; originalName: string }) => ({
              ...
              platform: "pinterest", campaignId: campaignId ?? null,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div
        ...
        style={{ border: `2px dashed ${isDragging ? ACCENT : "var(--color-border)"}`, borderRadius: 12, padding: "36px 20px", display: "flex", flexDirection: "column", alignItems: "center", gap: 10, cursor: "pointer", background: isDragging ? `${ACCENT}08` : "var(--color-surface-2)", transition: "all 200ms" }}
      >
        <div style={{ width: 48, height: 48, borderRadius: 12, background: `${ACCENT}18`, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <UploadCloud size={22} color={ACCENT} />
        </div>
        <p style={{ fontSize: 14, fontWeight: 700, color: "var(--color-text-primary)" }}>{isDragging ? "Drop files here" : "Drag & drop or browse"}</p>
        <p style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>Images or video — up to {MAX_MB} MB each</p>
        <span style={{ fontSize: 12, color: ACCENT, fontWeight: 600, padding: "5px 16px", border: `1px solid ${ACCENT}60`, borderRadius: 6, marginTop: 2 }}>Browse files</span>
      ...
      {queue.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: "var(--color-text-muted)", textTransform: "uppercase", letterSpacing: "0.06em" }}>{queue.length} file{queue.length !== 1 ? "s" : ""} queued</span>
            <button type="button" onClick={() => setQueue([])} style={{ fontSize: 11, color: "var(--color-text-muted)", background: "none", border: "none", cursor: "pointer" }}>Clear all</button>
          </div>
          {queue.map(qf => (
            <div key={qf.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", background: "var(--color-surface-2)", border: "1px solid var(--color-border)", borderRadius: 8 }}>
              ...
              <button type="button" onClick={() => setQueue(p => p.filter(q => q.id !== qf.id))} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--color-text-muted)", padding: 4, flexShrink: 0 }}>
                <X size={13} />
              </button>
            </div>
          ))}
      ...
      <button type="button" disabled={!canSubmit} onClick={handleUpload}
        style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "12px 20px", borderRadius: 9, border: "none", background: canSubmit ? `linear-gradient(135deg, ${ACCENT}, #ad081b)` : "var(--color-surface-2)", color: canSubmit ? "#fff" : "var(--color-text-muted)", fontSize: 13, fontWeight: 700, cursor: canSubmit ? "pointer" : "not-allowed", transition: "all 200ms", opacity: canSubmit ? 1 : 0.6 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, padding: 14, background: "rgba(34,211,160,0.06)", border: "1px solid rgba(34,211,160,0.18)", borderRadius: 10 }}>
          {results.map((r, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: r.status === "inserted" ? "#22d3a0" : "#f43f5e" }}>{r.status === "inserted" ? "Saved" : r.error}</span>
...
export function PinterestModal({ brandId, brandName, campaignId, campaignName, onClose, onSuccess }: Props) {
  ...
  return createPortal(
    <div
      role="dialog" aria-modal="true" aria-label="Add Pinterest creative"
      onClick={onClose}
    ...
    >
      <div onClick={e => e.stopPropagation()}
        style={{ width: "100%", maxWidth: 640, background: "var(--color-surface)", border: "1px solid var(--color-border)", borderRadius: 20, overflow: "hidden", display: "flex", flexDirection: "column", animation: "pinModalIn 240ms cubic-bezier(0.34,1.56,0.64,1) both" }}
      >
        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "18px 22px", borderBottom: "1px solid var(--color-border)", position: "sticky", top: 0, zIndex: 10, background: "var(--color-surface)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <p style={{ fontSize: 13, fontWeight: 700, color: "var(--color-text-primary)", lineHeight: 1.2 }}>Add Pinterest Creative</p>
        ...
        <div style={{ padding: "16px 22px 0", display: "flex", gap: 4 }}>
          {TABS.map(t => {
            ...
            return (
              <button key={t.id} id={`pin-tab-${t.id}`} onClick={() => setTab(t.id)}
      ...
      <style>{`
        @keyframes pinModalIn { from { opacity:0; transform:scale(0.97) translateY(16px); } to { opacity:1; transform:scale(1) translateY(0); } }
